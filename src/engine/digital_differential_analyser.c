...
/*   Updated: 2020/02/13 12:54:08 by mrouabeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void get_step(t_player *player, t_ray *ray)
{
	...
	{
		ray->step_x = -1;
		ray->side_dist_x = (player->pos_x -
							(int)player->pos_x) * ray->delta_dist_x;
	}
	else
	{
		ray->step_x = 1;
		ray->side_dist_x = ((int)player->pos_x +
							1.0 - player->pos_x) * ray->delta_dist_x;
	}
	if (ray->ray_y< 0)
	{
		ray->step_y = -1;
		ray->side_dist_y = (player->pos_y -
							(int)player->pos_y) * ray->delta_dist_y;
	}
	else
	{
		ray->step_y = 1;
		ray->side_dist_y = ((int)player->pos_y + 1.0 -
